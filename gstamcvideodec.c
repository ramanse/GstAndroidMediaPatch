    ...
    GstVideoCodecState * state)
{
  ...
  /*DAG Specific decoder-only settings for for low-latency streaming */
  GST_DEBUG_OBJECT (self, "DAG specific decoder settings");
  gst_amc_format_set_int (format, "low-latency", 0, &err);
  if (err)
    GST_ELEMENT_WARNING_FROM_ERROR (self, err);
  gst_amc_format_set_int (format, "max-width", state->info.width, &err);
  if (err)
    GST_ELEMENT_WARNING_FROM_ERROR (self, err);
  gst_amc_format_set_int (format, "max-height", state->info.height, &err);
  if (err)
    GST_ELEMENT_WARNING_FROM_ERROR (self, err);
